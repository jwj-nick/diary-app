...
export function GoalForm({ onSuccess, onCancel, entry, defaultDate }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      {/* Subject (optional) */}
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">과목 (선택)</label>
      ...
      {/* Target date */}
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">목표일</label>
        <input
          type="date"
          {...register('targetDate')}
          className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"
      ...
      {/* Description */}
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">상세 설명 (선택)</label>
        <textarea
      ...
      {/* Steps */}
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">세부 단계 (체크리스트)</label>
